...
def InceptionResNetV2(include_top=True,
    ...
    if input_tensor is None:
        img_input = inp.Input(shape=input_shape)
    else:
        if not backend.is_keras_tensor(input_tensor):
            img_input = inp.Input(tensor=input_tensor, shape=input_shape)
        else:
            img_input = input_tensor

    # Stem block: 35 x 35 x 192
    x = conv3d_bn(img_input, 32, 3, strides=2, padding='same')
    x = conv3d_bn(x, 32, 3, padding='same')
    x = conv3d_bn(x, 64, 3, padding='same')
    ...
    branch_0 = conv3d_bn(x, 384, 3, strides=2, padding='same')
    branch_1 = conv3d_bn(x, 256, 1, padding='same')
    branch_1 = conv3d_bn(branch_1, 256, 3, padding='same')
    branch_1 = conv3d_bn(branch_1, 384, 3, strides=2, padding='same')
    branch_pool = layers.MaxPooling3D(3, strides=2, padding='same')(x)
    ...
    branch_0 = conv3d_bn(x, 256, 1, padding='same')
    branch_0 = conv3d_bn(branch_0, 384, 3, strides=2, padding='same')
    branch_1 = conv3d_bn(x, 256, 1, padding='same')
    branch_1 = conv3d_bn(branch_1, 288, 3, strides=2, padding='same')
    branch_2 = conv3d_bn(x, 256, 1, padding='same')
    branch_2 = conv3d_bn(branch_2, 288, 3, padding='same')
    branch_2 = conv3d_bn(branch_2, 320, 3, strides=2, padding='same')
    branch_pool = layers.MaxPooling3D(3, strides=2, padding='same')(x)
    branches = [branch_0, branch_1, branch_2, branch_pool]
